// BizAssist_mobile
// path: src/modules/categories/screens/InventoryCategoryDetailScreen.tsx
//
// Header governance:
// - Category details is an Inventory detail screen -> use BACK.
...
export default function InventoryCategoryDetailScreen() {
	...
	const onEdit = useCallback(() => {
		...
		if (!lockNav()) return;
		router.push(`/(app)/(tabs)/inventory/categories/${encodeURIComponent(category.id)}/edit` as any);
	...
	const onOpenItem = useCallback(
		(productId: string) => {
			...
			if (!lockNav()) return;
			router.push(
				`/(app)/(tabs)/inventory/categories/${encodeURIComponent(id)}/items/${encodeURIComponent(productId)}` as any,
			);
		},
